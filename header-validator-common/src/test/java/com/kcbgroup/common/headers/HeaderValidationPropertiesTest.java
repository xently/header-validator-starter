    ...
    class PropertiesLoading {
        ...
        void shouldBindPropertiesCorrectly() {
            runner.withPropertyValues(
                    "kcb.api.headers.validation.headers.[0].header-name=X-Custom-Header",
                    "kcb.api.headers.validation.headers.[0].required=true",
                    "kcb.api.headers.validation.headers.[0].validator=com.kcbgroup.common.headers.validators.EpochTimestampValidator"
            ).run(context -> {
                assertThat(context)
        ...
        void shouldDefaultToRequiredAndDefaultValidator(TestCase testCase) {
            runner.withPropertyValues("kcb.api.headers.validation.headers.[0].header-name=X-Custom-Header").run(context -> {
                assertThat(context)
                        .hasSingleBean(HeaderValidationProperties.class);
